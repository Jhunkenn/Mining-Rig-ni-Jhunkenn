...
export default function App() {
  ...
  const [chkOpen, setChkOpen] = useState(false);     // checker status detail popover
  ...
  const checkerAgeText = libAgeMin == null ? "—" : libAgeMin === 0 ? "just now" : libAgeMin + "m ago";
  // compact tertiary status: icon + "Checker" only (entry count lives in the popover/hover, not the header)
  const checkerState =
    !checkerConfigured ? { icon: "○", label: "Off", tone: "var(--ink-soft)" }
    : libStatus === "loading" ? { icon: "○", label: "Connecting…", tone: "var(--ink-soft)" }
    : libStatus === "unavailable" ? { icon: "○", label: "Unavailable", tone: "var(--accent)" }
              ...
              <span ref={chkRef} onMouseEnter={openChk} onMouseLeave={scheduleCloseChk} style={{ position: "relative", display: "inline-flex", alignItems: "center" }}>
                ...
                {chkOpen && (
                  <div style={{ position: "absolute", top: "calc(100% + 6px)", right: 0, zIndex: 30, minWidth: 170, background: "#ffffff", border: "1px solid var(--note-line, #e2ddd0)", borderRadius: 10, padding: "9px 11px", boxShadow: "0 8px 24px rgba(0,0,0,.16)", textAlign: "left", cursor: "default" }}>
                    <div style={{ fontSize: 11.5, fontWeight: 800, color: "var(--note-ink)", marginBottom: 5 }}>Library Checker</div>
                    <div style={{ fontSize: 11, color: "var(--note-ink)", lineHeight: 1.6 }}>
